...
def _forget_resolved_flavor():
    """Legacy cache state must not leak between tests."""
...
def test_v3_session_creation_sends_playbook_tags_acu_limit_and_schema():
    ...
    state = client.create_session(
        ...
        repos=["https://github.com/yacine-baghli/DYB-Pro"],
        secret_ids=["secret-gpu"],
    )
    ...
    assert body["repos"] == ["https://github.com/yacine-baghli/DYB-Pro"]
    assert body["secret_ids"] == ["secret-gpu"]
    assert state.session_id == "devin-abc"
    assert state.status == "running"
    assert not state.is_terminal


def test_v3_permission_error_is_not_retried_against_legacy_v1():
    """A 403 means fix service-user RBAC, never send a cog_ credential to legacy v1."""
    calls: list[str] = []
    ...
    client = _client(handler, devin_api_key="cog_service_user")
    with pytest.raises(DevinAPIError) as excinfo:
        client.create_session("design something", title="orchestrator")
    assert excinfo.value.status_code == 403
    assert calls == ["/v3/organizations/org-123/sessions"]
    assert client.api_flavor() == "v3"
...
def test_session_state_terminality_and_structured_output():
    ...
    suspended = _client(
        lambda request: httpx.Response(200, json={"session_id": "devin-suspended", "status": "suspended"})
    ).get_session("devin-suspended")
    assert suspended.is_terminal and suspended.failed
...
def test_health_surfaces_v3_rbac_errors_without_downgrade():
    """Provider health must expose a permissions problem instead of hiding it behind v1."""
