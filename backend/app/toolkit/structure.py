...
def burial(structure: Structure, cutoff: float = 10.0) -> list[int]:
    """Neighbour count per residue — a cheap, monotone proxy for burial/SASA."""
    coords = structure.coords
    counts = []
...
def relative_exposure(structure: Structure) -> list[float]:
    ...
    if not counts:
        return []
    hi = max(counts) or 1
    return [round(1.0 - c / hi, 3) for c in counts]
...
def compactness(structure: Structure) -> float:
    """Rg normalized by the empirical Rg ~ 2.2 * N^0.38 (Skolnick, 2011). <1 = compact."""
    n = len(structure.coords)
    expected = 2.2 * (n**0.38)
